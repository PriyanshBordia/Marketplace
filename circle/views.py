...
def addMessage(request, chat_id):
	try:
		if request.POST:
			...
			text = Message.objects.create(sender=sender, receiver=receiver, text=message)
...
@login_required
def chats(request):
	chats = Chat.objects.filter(Q(left=request.user.person) | Q(right=request.user.person))
