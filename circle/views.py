        ...
        if request.POST:
            ...
            if form.is_valid():
                ...
                if Person.objects.filter(email=email).exists():
                    return render(request, "circle/error.html", context={"message": "Email Already Registered.!!",
    ...
    try:
        ...
        if Person.objects.filter(pk=request.user.person.id, friends__pk=person.id).exists():
            if Chat.objects.filter(Q(left=person_id, right=request.user.person.id) | Q(left=request.user.person.id, right=person_id)).exists():
                chat_id = Chat.objects.filter(Q(left=person_id, right=request.user.person.id) | Q(
                    left=request.user.person.id, right=person_id)).first().id
        ...
        return render(request, "circle/error.html", context={"message": "Person Does Not Exist.!!",
                                                             "type": "Data Error.!",
                                                             "link": "search"})
    except Chat.DoesNotExist:
        return render(request, "circle/error.html", context={"message": "No Chat Found.!!",
                                                             "type": "Data Error.!",
                                                             "link": "search"})
...
def addArticle(request):
    ...
    try:
        ...
        return render(request, "circle/error.html", context={"message": str(e), "type": "Internal Error", "link": "newArticle"})
...
def edit(request, id, type):
    if type == 'person':
        ...
        try:
            ...
            return render(request, "circle/error.html", context={"message": "No Tag Found.!!", "type": "Type Error", "link": "tags"})
...
def addTag(request):
    ...
    try:
        ...
        return render(request, "circle/error.html", context={"message": str(e), "type": "Internal Error", "link": "addTag"})
...
def tag(request, tag_id):
    try:
        ...
        return render(request, "circle/error.html", context={"message": "Tag does not exist.!!", "type": "Type Error", "link": "search"})
...
def friends(request):
    try:
        ...
        return render(request, "circle/error.html", context={"message": "You don't have any friends.!!",
                                                             "type": "Data Error",
                                                             "link": "search"})
    except Chat.DoesNotExist:
        return render(request, "circle/error.html", context={"message": "You don't have any chats.!!",
                                                             "type": "Data Error",
                                                             "link": "search"})
...
def result(request, type):
    try:
        ...
        try:
            ...
            if type == 'article':
                # .values_list('id'))
                display = list(Person.objects.get(
                    pk=request.user.person.id).display.all())
                # .values_list('id'))
                rent = list(Person.objects.get(
                    pk=request.user.person.id).rented.all())
                # .values_list('id'))
                purchased = list(Person.objects.get(
                    pk=request.user.person.id).purchased.all())
                # .values_list('id'))
                sold = list(Person.objects.get(
                    pk=request.user.person.id).sold.all())
            ...
            if type == 'person':
                friends = list(Person.objects.get(
                    pk=request.user.person.id).friends.all())
                persons = Person.objects.filter(Q(first__contains=search) | Q(
                    last__contains=search) | Q(username__contains=search)).exclude(friends__in=friends)
